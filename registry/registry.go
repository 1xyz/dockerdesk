...
func (r *Registry) push(ctx context.Context, ui terminal.UI, image *docker.Image) (*Artifact, error) {
	u := ui.Status()
	defer u.Close()
	u.Update("Pushing binary to registry")
	u.Update(fmt.Sprintf("image information: %v:%v location:%v",
		image.Image, image.Tag, image.Location))
	return &Artifact{}, nil
}
